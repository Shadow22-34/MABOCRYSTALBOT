...
class ScriptDB:
    ...
    async def update_script(self, script_name, script_data):
        data = self._load_data()
        if script_name not in data["scripts"]:
            raise ValueError(f"Script {script_name} not found")
        
        data["scripts"][script_name].update(script_data)
        data["scripts"][script_name]["lastUpdated"] = datetime.now().strftime("%Y-%m-%d")
        data["metadata"]["lastUpdate"] = datetime.now().isoformat()
        self._save_data(data)
        return data["scripts"][script_name]

    async def get_script(self, script_name):
        data = self._load_data()
        return data["scripts"].get(script_name)
